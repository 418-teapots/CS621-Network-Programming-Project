 ...
 */
#ifndef REQUEST_RESPONSE_HELPER_H
#define REQUEST_RESPONSE_HELPER_H
 ...
 */
class RequestResponseServerHelper
{
public:
  /**
   ...
   */
  RequestResponseServerHelper (uint16_t port);
...
public:
   ...
   */
  RequestResponseClientHelper (Address ip, uint16_t port);
  /**
   * Create RequestResponseClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  RequestResponseClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a RequestResponseClient application, set the data fill of the
   ...
   *
   * \param app Smart pointer to the application (real type must be RequestResponseClient).
